...
class Model():
    def __init__(self, C, height, width, gradient_checkpointing=False):
        ...
        #changing Hardtanh values to discrete values by using discretization
        ...
        if not gradient_checkpointing:
            grads_and_vars = list(zip(encoder_grads+decoder_grads, encoder_params+decoder_params))
            self.train_op = tf.train.AdamOptimizer().apply_gradients(grads_and_vars)
        else:
            #Gradient checkpointing to lower down the training load on GPU
    ...
    def build_net(self):
        ...
        height = self.height
        width = self.width
        #encoder layers -- downsample C channel layers
        with tf.variable_scope('encoder'):
            #layer 1: CNN layer with 2*2 strides on 32 channels and filter of 3*3
            ...
            #Applying RELU Activation to add non linearity, better than tanh
